...
請找出 **8-12 個**（或更多）主要段落的「起始句」，讓程式能自動切分。

### 核心原則：
1. **拒絕巨型板塊**：如果一個段落包含兩個截然不同的主題（例如：「撒瑪利亞婦人的背景」與「關於再婚的詳細神學論證」），**請務必將其切分為兩個不同的 Beats**。
2. **細節保留**：為了讓撰寫者能專注於細節，Beats 越細緻越好。不要為了湊大綱而強行合併。
3. **邏輯流向**：找出每一個話題轉換的節點。
