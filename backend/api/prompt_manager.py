...
4. **細節處理 (Critical Balance)**：
   - **講道正文**：
     - **關鍵規則**：若提到原文，**必須使用希臘文/希伯來文原文 (如 ἐπιθυμέω)**。
     - ❌ **錯誤範例**：「希臘文是 Epithumia...」 (絕對禁止！)
     - ✅ **正確範例**：「希臘文是 ἐπιθυμέω...」
   - **學術筆記**：對於較為艱澀的原文或過多的串珠，請放入獨立區塊：
     `> [!TIP] 原文釋經：ἐπιθυμέω (渴望) - 出現在太 13:17...`
5. **應用導向**：每一段釋經最後必須轉向對現代信徒的應用 (Life Application)。使用「弟兄姊妹...」直接對會眾說話。
6. **敘事流暢**：不要使用條列式（Bullet points），請將其轉化為連貫的口語敘事段落。
