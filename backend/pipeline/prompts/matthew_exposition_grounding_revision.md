...
5. 返回完整 Markdown 與完整 ledger。ledger 的每個 section 必須原樣帶回初稿already有的全部欄位，一個都不能少：
   ...
   `section_id`、`decision_ids`、`claim_ids_used`、`viewpoint_revision_ids_used`、`applied_operations`、`integration_operations`、`omissions`、`output_anchor`
...
修掉一句越界的話時，不要用來源說明去補位。「材料中的讀音轉寫為⋯⋯」「母本另有說明」這類寫法把編輯部的內部結構寫給了讀者，正文、標題與註釋一律不可。刪掉沒有材料的那半句就好，讀者不需要知道剩下那半句出自哪裡。
