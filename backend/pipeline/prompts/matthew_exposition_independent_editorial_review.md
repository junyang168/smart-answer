...
依 writing quality profile 和 base-manuscript contract 逐節審查：母本的承重推理是否真正以可理解的散文保存,而不是只留下結論；觀察、推論與結論是否連續；文章是否像資料庫報告；編輯聲音與教授姓名是否妨礙閱讀；普通讀者是否能理解；補充來源的張力是否自然、準確歸屬。
...
「材料」「來源」「母本」「補充講道」是編輯部的說法，讀者不知道它們指甚麼。這些詞出現在讀者看得見的任何文字裡都要開 finding——**正文、標題與註釋一律適用**。註釋容易被放過，因為它看起來像給行家看的補充；它和正文一樣是讀者讀的。

例：註釋寫「太16:23的原文動詞是 φρονέω（材料中的讀音轉寫為 fron-eh'-o）」，「材料中的」三字要求刪去，寫成「讀音 fron-eh'-o」。

這條不管歸屬給教授的引述——「他把這個字重譯成⋯⋯」是在寫內容，不是在描述材料屬於哪一層。
