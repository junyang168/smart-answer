...
5. 每个实质段落都要在前一行写单行 JSON provenance：`<!-- provenance: {"attribution":"professor","claim_ids":["..."],"evidence_step_ids":["..."],"argument_route_revision_ids":["..."],"texture_anchors":[{"source_id":"...","excerpt":"..."}]} -->`（`texture_anchors` 可选，用法见规则 6a–6c）。`evidence_step_ids` 必须逐项列出本段实际使用的 Evidence Step，且每一项都属于本段声明的 Claim；它控制来源预览中补充原文的范围，不能把一个 Claim 的全部 Evidence Step 粗略塞给整段。凡本段在展开推理或阶段结论，`argument_route_revision_ids` 必须列出本段实际使用且属于当前 section 的路线；只作经文引述或不依赖路线的简单陈述时使用空数组。来源预览将沿 route attestation 的 step bindings 展示论证骨架，并用本段声明的 Evidence Step 补足逐字引文或叙事前提；两者互补，不能互相代替。跨来源的编辑归纳使用 `editorial_synthesis`，同时列出实际支撑它的 Claim、Evidence Step 和路线；不得把编辑归纳写成教授原话。
6. 每个段落的断言都必须能回到该段 `claim_ids` 的 Claim、Evidence 与 source excerpt。不要补材料没有的心理、因果、调和、背景或一般原则。
6a. 接地有两条路，按句子做的事分流。**下结论**——释经判断、推论、神学主张——必须走 Claim 路，一如既往。**还原教授怎么教**——他的讲法框架、比喻、字词解释、事件时间、地点背景、亲口重译——可以走 texture 路：在该段 provenance 加 `texture_anchors`，每项给出 `source_id` 与教授原文的**逐字** `excerpt`（`knowledge.source_originals.originals[].content` 的连续片段，至少 10 字）。程序会逐字核对，对不上整份结果被拒。判断方法写成一个问题：这句话在下结论，还是在还原教授怎么教？
6b. 材料里有教授自己的具体讲法时，**优先采用**，用 texture anchor 锚定，不要因为它没有对应 Claim 就改写成抽象转述。「门徒通过第一课的考试，耶稣开始教他们第二课」胜过「教导进入第二个阶段」；教授亲口的重译、比喻和地点说明是文章生动的来源，逐字稿里有而你不用，是本 prompt 明确反对的写法。消化课堂口语的夸张与斥责语气后再入文，锚定的是内容，不是照抄语气。
6c. texture 只承载教学血肉，绝不偷带结论。锚文之外的判断、因果、动机与神学推论仍须 Claim 支持；纯叙事段落可以只有 `texture_anchors` 而无 `claim_ids`，但这样的段落不得使用 `evidence_step_ids` 或 `argument_route_revision_ids`——它不在做论证。若一句话既还原教学又下结论，结论部分照常声明 Claim，texture anchor 只为其中的教学细节负责。
