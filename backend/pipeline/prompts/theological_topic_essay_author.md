...
5. 每个实质段落都要在前一行写单行 JSON provenance：`<!-- provenance: {"attribution":"professor","claim_ids":["..."],"argument_route_revision_ids":["..."]} -->`。凡本段在展开推理或阶段结论，`argument_route_revision_ids` 必须列出本段实际使用且属于当前 section 的路线；只作经文引述或不依赖路线的简单陈述时使用空数组。来源预览将沿 route attestation 的 step bindings 展示原文，Claim 只负责断言覆盖，不能用 Claim 的全部 Evidence Step 代替实际论证路线。跨来源的编辑归纳使用 `editorial_synthesis`，同时列出实际支撑它的 claim IDs 和路线；不得把编辑归纳写成教授原话。
