...
判断之外还要给一个 `disposition`，说明这条边接下来该怎么办。写在 reason 里不算——散文没人能照着执行，那条边就会一直留在库里，看上去和通过的没有区别。

| disposition | 用在什么时候 |
|---|---|
| `keep` | 通过（`decision: pass` 且结构化问题为 true）。只有这种情形能填 |
| `retype` | 两端次序对，但类型说不准。**必须同时给 `suggested_relation_type`** |
| `retire` | 这条边不承重，或没有任何准确的类型可换。停用它 |
| `needs_human` | 方向记反了 |

**方向反了一律 `needs_human`。** 调转两端等于重新判断两个观点如何相对而立，随之要选的类型也是判断——这不是一条有唯一答案的更正，不由批处理来做。

若这条边只是主题相邻、不承重，`retire` 是正当答案：主题相邻应由 structure 的 focal 共现承担，而不是伪装成一条有向关系。
