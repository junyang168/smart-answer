...
def body_coordinate_sha256(script: Any) -> str:
    """Identify the body-row coordinate skeleton without treating edits as source.

    The review copy intentionally omits publication-only timing fields.  Its
    editorial headings can still be applied to the published transcript when
    the ordered paragraph identities agree.  Text is deliberately absent from
    this key: the authoritative wording comes from the published document, not
    from the editor's co-located working copy.
    """

    rows = project_script(script).body_rows
    return _sha256_json(
        [
            {
                "index": row.get("index"),
...
def apply_editorial_structure(source_script: Any, editorial_script: Any) -> list[dict[str, Any]]:
    """Overlay editor-authored headings onto authoritative spoken rows.

    Storage co-location never makes a subtitle part of the professor's source.
    This projection keeps every body row (including timing) from ``source_script``
    and imports only heading boundary, level, and title from ``editorial_script``.
    A coordinate mismatch fails closed before model work.
    """

    source = project_script(source_script)
    editorial = project_script(editorial_script)
    if body_coordinate_sha256(source_script) != body_coordinate_sha256(editorial_script):
        raise ValueError(
            "editorial transcript body coordinates do not match the authoritative source"
        )
    if [row.get("text") for row in source.body_rows] != [
        row.get("text") for row in editorial.body_rows
    ]:
        raise ValueError(
            "editorial transcript body text does not match the authoritative source"
        )
    if any(row.boundary < 0 or row.boundary > len(source.body_rows) for row in editorial.headings):
        raise ValueError("editorial heading boundary is outside the authoritative source")

    by_boundary: dict[int, list[EditorialHeading]] = {}
    for heading in editorial.headings:
        by_boundary.setdefault(heading.boundary, []).append(heading)

    combined: list[dict[str, Any]] = []
    heading_index = 0
    for boundary in range(len(source.body_rows) + 1):
        for heading in by_boundary.get(boundary, []):
            heading_index += 1
            combined.append(
                {
                    "index": f"editorial-overlay-{heading_index}",
                    "type": "subtitle",
                    "text": f"{'#' * heading.level} {heading.title}",
                }
            )
        if boundary < len(source.body_rows):
            combined.append(dict(source.body_rows[boundary]))
    return combined
