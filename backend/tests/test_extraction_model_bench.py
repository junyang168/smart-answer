...
def test_cost_sums_rejected_attempts_and_discounts_cached_input():
    """A package that needed three tries cost three calls; all of them count."""

    row = bench.row_from_package(package(extraction={"model_id": "gpt-5.6-sol"}),
                                 model="gpt-5.6-sol",
                                 reasoning_effort="medium", elapsed_seconds=1.0)

    assert row.prompt_tokens == 2000
    assert row.cached_tokens == 900
    assert row.completion_tokens == 1000
    # 1100 fresh @5.00 + 900 cached @0.50 + 1000 output @30.00, per million.
    # Compared to the reported precision rather than to an exact float:
    # `price_usage` sums per call and rounds to four places, so recomputing
    # the total in one expression lands on the other side of the boundary.
    assert row.cost_usd == pytest.approx(
        (1100 * 5.00 + 900 * 0.50 + 1000 * 30.00) / 1_000_000, abs=1e-4
    )
    assert row.cost_complete is True


def test_a_candidate_has_no_price_until_production_adopts_it():
    """Unpriced is the honest answer, and it must not read as free.

    The price table carries the models production runs. A candidate under
    evaluation is not in it, so its cost is None with a note -- never 0.0,
    which would sum into totals as though the run were free.
    """

    row = bench.row_from_package(package(extraction={"model_id": "kimi-k3"}),
                                 model="kimi-k3",
                                 reasoning_effort="low", elapsed_seconds=1.0)
...
def test_deepseek_gets_the_token_cap_it_actually_reads():
    ...
    exactly 200 with finish_reason `length`. A silently dropped cap is how one
    section produced 40,938 completion tokens against a 16,000 budget.
    """

    assert bench.CANDIDATES["deepseek"]["token_limit_param"] == "max_tokens"
...
def test_deepseek_falls_back_to_json_object_because_json_schema_is_disabled():
    ...
    assert bench.CANDIDATES["deepseek"]["response_format"] == "json_object"
...
def test_kimi_carries_its_own_quirks_instead_of_the_production_registry():
    """Trying a model must not require editing the code that runs production.

    kimi-k3 needs a fixed temperature, a reasoning setting, a smaller budget
    and streaming. All four live in this file; `MODEL_BACKENDS` does not know
    the model exists.
    """

    from backend.pipeline.detailed_knowledge_extraction_runner import MODEL_BACKENDS

    kimi = bench.CANDIDATES["kimi"]
    assert kimi["temperature"] == 1.0
    assert kimi["pass_reasoning_effort"] is True
    assert kimi["stream"] is True
    assert kimi["max_output_tokens"] < 64000
    assert "kimi" not in MODEL_BACKENDS


def test_a_production_model_is_benched_through_the_production_client(monkeypatch):
    """gpt is measured as production runs it, or the measurement is of us."""

    seen = {}

    def spy(model, *, reasoning_effort, max_output_tokens):
        seen.update(model=model, effort=reasoning_effort, budget=max_output_tokens)
        return object()

    monkeypatch.setattr(bench, "build_client", spy)
    bench._bench_client("gpt-5.6-sol", reasoning_effort="medium", max_output_tokens=None)

    assert seen["model"] == "gpt-5.6-sol"
    assert seen["budget"] == bench.DEFAULT_MAX_OUTPUT_TOKENS
