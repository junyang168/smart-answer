...
def test_editorial_scope_is_sha_bound():
    ...
    with pytest.raises(TheologicalEvidencePacketError, match="SHA mismatch"):
        validate_editorial_scope(scope)


def test_evidence_compiler_follows_the_complete_structure():
    packet = _compile()
    assert packet["compiler_readiness"] == "ready_for_authoring"
    assert {
...
def test_evidence_compiler_rejects_a_stale_structure_revision():
    ...
    with pytest.raises(TheologicalEvidencePacketError, match="active/current"):
        _compile(records)


def test_dry_run_entry_writes_the_compiled_packet(tmp_path, monkeypatch, capsys):
    scope_path = tmp_path / "scope.json"
    scope_path.write_text(json.dumps(_scope()), encoding="utf-8")
    packet = _compile()
    monkeypatch.setattr(
        theological_evidence_packet_runner,
        "compile_from_store",
        lambda *, scope: packet,
    )
    monkeypatch.setattr(
    ...
    assert theological_evidence_packet_runner.main() == 0
    written = json.loads(
        (tmp_path / "packet" / "theological-evidence-packet.json").read_text(
            encoding="utf-8"
        )
    )
    assert written["evidence_packet_sha256"] == packet["evidence_packet_sha256"]
