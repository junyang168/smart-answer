...
**其他解决方案 A**：合并式 MDM——从多个来源挑出或拼出一条「黄金记录」，来源退居其次。

**为什么不用**：合并等于把教授某一次的措辞提升为「标准原话」，其余抹掉。用在姓名地址上合理（挑一个最完整的地址即可），用在他的话上就是误述教授。观点层保存的是审核过的编辑归一化文字，**它不冒充教授原话**，来源主张一条不少地留着。

**其他解决方案 B**：逐条生成、逐条比对——一条条产出观点节点，每产出一条就去和已有的比对、决定合并还是新建。这是传统 MDM 的路子，**本项目建过并否决了**。

**为什么不用**：**卡在 blocking 与 recall 上。**

逐条比对绕不开一个数量问题：一千多条主张两两比是上百万次，跑不动。传统做法是先**分块**（blocking）——按某个键把可能相同的放进同一块，只在块内比。而**任何分块都在丢 recall**：教授在另一篇讲道里换个说法讲同一件事，落进别的块，就永远不会被拿来比。丢掉的那条不会报错，它只是不存在——这正是 D2 里那种「读起来完整」的失误，换了个地方出现。

调紧分块就跑不动，放松分块就丢得更多，而**丢了多少你不知道**，因为没有 gold set 可以量。

现在的做法是把 blocking 降级：它只负责**提候选**，不决定身份。候选集是规则、Claim embedding 与 signature embedding 的 **lossless union**——一路漏掉的由另一路补上；身份留到后面在整张图上定。

第二个理由：它押在最不可靠的那个判断上。#211 的盲测把两个强模型放在同一份材料上各自独立重建，结果是：
...
**「哪两条命题是同一个」恰恰是逐条比对唯一依赖的那个判断，而它是最不稳的。** 实测里两个模型都把「磐石不是彼得」和「教会不是建立在彼得身上」留作两个节点——逐条提升就会把一个观点劈成两个。

结论是 `GO WITH CHANGE`：改为先重建整张论证图，再从图上定身份。原话是「raw proposition nodes are not Registry-ready……do not promote model-local nodes one-for-one」（#211，2026-08-24；后续修法随 #215 合并）。
