...
> **记录王教授的观点和论证，而且可查证、可回溯。**

它内部恰好三个 component，因为这句话有三段：
...
那换成让 AI 直接读这堆文档（RAG）呢？更不行，而且坏在要害上：

- 它**会发明**。模型从检索到的段落里生成一段读起来很有道理的话，而那句话教授从没讲过——这恰恰是第 6 节里最坏的结果。
- 它**无法核对**。答案是当场生成的，没有稳定的对象可以指认、可以人工批准、可以在下次追问时给出同一个答案。
- 它**留不下东西**。这一次的推理不会沉淀，下一个问题从零开始，第 5.1 条要压的那个数一次也没降。
