...
flowchart TD
    ...
    subgraph L1["来源层：对来源的记录"]
        SRC["source_documents：哪一份来源、什么类型、内容哈希<br/>source_fragments：教授原话的逐字片段、录音时间、锚定状态"]
    end

    subgraph L2["知识层"]
        CLAIM["Claim：单篇之内的主张、经文依据与推论步骤"]
        CVP["CanonicalViewpoint：跨讲道的观点身份与论证路线"]
        CLAIM --> CVP
    end
    ...
    CVP --> TOPIC
    CVP --> QA
    L3 -.-> OPT["optional：微讲道、搜索、比较、课程、学术思想整理"]
```

| 层 | 它保存什么 | 不要它会怎样 |
| --- | --- | --- |
| **来源层** | **对来源的记录**：每一段教授原话的逐字文字、它在录音里的时间、以及内容哈希 | 无法回答「你怎么知道教授这样说」。这是使命层的底线，不可谈判 |
| **知识层** | 教授的主张、经文依据、推论步骤，以及这些主张跨讲道的归属与论证路线 | 只能从来源直接写文章。那样无法回答「他别处讲过吗」，文章会变成唯一权威，问答只能从文章里反猜主张。详见[知识层详细设计](./knowledge_platform_design.md) |
| **应用层** | 文章、主题论述、问答 | —— |

### 来源层：它保存的是对来源的记录，不是来源本身

教授的视频、音频和释经课笔记在**系统之外**。系统里保存的是对它们的记录，两类：

| collection | 它保存什么 |
| --- | --- |
| `source_documents` | 这是哪一份来源、什么类型、内容哈希是多少 |
| `source_fragments` | **教授原话的逐字片段**、它在录音里的**时间位置**、锚定是否成功 |

第 2 节承诺给外教会读者的「每一句都能当场看到它出自哪里」，兑现在这一层：`source_fragments` 里的逐字片段加录音时间，就是他核对用的东西。

**为什么它是一层，而不是知识层里的又一个 component。** 尽管它和 Claim 存在同一个库里：

> **来源记录是「他说过这句话」的事实；Claim 是「这句话是什么主张」的解释。**

弄错来源是**数据错误**——可以逐字比对、比对哈希，机器就能查出来。弄错 Claim 是**解释错误**——需要判断。这条界线直接决定什么能自动化、什么必须转人工（第 5.3 条），所以它值得画成一道边界，而不是埋在同一层里。
...
知识层内部有两样东西，职责不同，缺一样都答不了第 1 节那两个问题。

| | **Claim** | **CanonicalViewpoint** |
| --- | --- | --- |
| 范围 | **单篇之内** | **跨讲道** |
| 它是什么 | 教授在这一篇里回答什么问题、提出什么主张、引什么经文、怎样从经文推到结论、反对哪种解释 | **观点本身**，以及它在不同讲道里各自的论证路线 |
| 一句话 | 「某篇讲道里的一次表达」 | 「他的观点」 |
...
- 只有 Claim，没有 CanonicalViewpoint——只能说「这 12 条主张彼此相关」，不能说「教授对因信成义的看法是」。第 1 节那两个问题恰恰只能用后者回答。而且新讲道进来时无处可挂，只能重新归组一遍，这与第 5.2 条冲突。
- 只有 CanonicalViewpoint，没有 Claim——观点悬空，无法逐句回到教授的原话，第 1 层的底线就没了。
...
| 层 | 它挡住什么 |
| --- | --- |
| **来源层** | 「他到底有没有说过」——每条主张连回原话、笔记页码或录音时间 |
| **知识层** · Claim | 「这是他说的，还是从他的话推出来的，还是编辑归纳的」——四种归属必须分开标注：教授明确主张、论证结论、编辑归纳、待研究问题 |
| **知识层** · CanonicalViewpoint | 「把两个不同的观点误合成一个」——归并需要证据；宁可分开并标记张力，不做表面调和 |
| **应用层** | 「文章为了流畅删掉限制条件或不同意见」——出版体例与程序化审计 |

知识层挡的两种失误方向相反：Claim 那一格防的是**把编辑的话说成教授的**，CanonicalViewpoint 那一格防的是**把教授的两个意思揉成一个**。所以它们不能合并成一道检查。
