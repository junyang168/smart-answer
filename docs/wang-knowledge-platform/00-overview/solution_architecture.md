...
*人工那头*：每一对候选都要人判，队列随语料线性增长。这与第 2 节「每新增一篇讲道的人工决定次数趋近于零」直接冲突——语料还在增加，处理得越多积压越多。

*召回那头*：一千多条主张两两比是上百万次，跑不动，所以必须分块；而**任何分块都在丢 recall**。教授在另一篇讲道里换个说法讲同一件事，落进别的块，就永远不会被拿来比。丢掉的那条不会报错，它只是不存在——这正是 D2 里那种「读起来完整」的失误换了个地方出现。调紧分块跑不动，放松分块丢得更多，而**丢了多少你不知道**，因为没有 gold set 可以量。

还有第三点，#211 的盲测量了出来：逐对比对押在最不可靠的那个判断上。两个强模型在同一份材料上各自独立重建，一致度是——
...
**「哪两条命题是同一个」恰恰是逐对比对唯一依赖的判断，而它最不稳。** 实测里两个模型都把「磐石不是彼得」和「教会不是建立在彼得身上」留作两个节点——逐条提升就会把一个观点劈成两个。结论是 `GO WITH CHANGE`：先重建整张论证图，再从图上定身份（#211，2026-08-24；修法随 #215 合并）。

**现在的做法：不做 blocking，也不生成候选对。** 这两样东西整个退出了生产路径——规则 blocking、signature、RecallGraph 只保留为历史诊断与回归用例，「不再是生产必经的领域状态」。

增量生产只做一件事：**取来相关的已有观点，给提议模型当 context**（`bounded relevant-viewpoint retrieval`）。取材这一步**不能建立任何东西**——设计原文：「任一通道只能扩大『值得给 proposer 看』的 context，不能建立 member、relation、CanonicalViewpoint 或 approval」。身份一律由上面那四步在整张论证图上定。

规则／signature／RecallGraph 的 lossless union 仍然存在，但只在 bootstrap 校准、盲测和**漏项审计**里跑——用来回答「我们漏了什么」，不参与产出。
