...
- **母本链接的 SVG 是独立视觉来源，不是加工后的正文。** `SourceDocument.source_path` 与文字 fragment 继续绑定原始 `final.md`；Markdown 图片链接保留在该正文中。每个视觉 fragment 则必须同时绑定图片链接所在的 body locator、实际 SVG 的 `source_path`、物理文件 SHA 与视觉 block SHA。抽取 projection 可以为模型临时组合 Markdown 与 SVG，但 SVG bytes 不得写进 manuscript body identity，也不得把加工版 manifest 冒充原始来源。review 和最终 supersede 写库边界都必须重新读取当前 `final.md` 与 SVG：文字 body、链接、路径或任一 SHA 不一致就 fail closed，并重新抽取；不能沿用旧 package 或旧模型响应。
