...
flowchart TD
    ...
    CL["worker 认领：写下「我拿走了，15 分钟内有效」<br/>同 scope 的多个 job 合并成一个工作单元<br/>过期的观点版本换成当前版本"]
...
| | |
| --- | --- |
| **合并** | 三批观点各入了一个 job，worker 领取时会把**同一轮范围内**的合并成**一个**工作单元，并把队列里过期的观点版本换成 Registry 当前版本——不会照着三份过期切面各跑一遍 |
| **崩溃可恢复** | 认领时写下一个有效期（默认 15 分钟）。正常干完就标记完成；**中途崩了没人改状态，有效期一到就过期，下一个 worker 可以重新领走**。若只标「进行中」而没有有效期，这个 job 会永远卡住，要人去清。这不算语义异常，也不改动那个不可变的 job 记录 |
| **边界** | 一个工作单元不得跨两轮的主张清单、证据范围或模型 policy。**两轮的清单各自冻结过，混着做就说不清做的是哪一批。** 要跨，就是两个工作单元 |
