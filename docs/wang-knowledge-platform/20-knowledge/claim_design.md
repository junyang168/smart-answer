...
| 对象 | 它是什么 | 代码里 |
| --- | --- | --- |
| **来源文档** | 一份来源的**全部**——整篇讲道或整份母本，一份一条记录。库里只有标题、类型、文件路径和整份文件的内容哈希，正文在文件系统上 | `source_documents` |
| **来源片段** | 其中的**一部分**——被引用到的那一段逐字文字，带段落编号和录音起止时间 | `source_fragments` |
| **Claim** | 教授的一个判断 | `claims` |
| **出现**（图中 `CLAIM_OCCURRENCE`） | 这个判断在某一篇的某一处被讲到，带原话高亮、行号、录音时间。**它不是独立的表**，是主张记录里的内嵌数组 | `claims.occurrences[]` |
...
```mermaid
flowchart TD
    T["完整逐字稿或母本<br/>（同工已校对）"] --> E["提议：逐句详细整理<br/>gpt-5.6-sol"]
    E --> M["程序机械验证<br/>锚点能否逐字匹配、经文能否解析、证据是否齐"]
    M -->|"不通过"| E
    M -->|"通过"| C["独立复核<br/>claude-sonnet-5"]
    C -->|"无异议"| K["写入知识库<br/>review_status = candidate"]
    C -->|"有异议"| O["仲裁：意见交回提议者<br/>gpt-5.6-sol"]
    O -->|"接受并改正"| K
    O -->|"仍谈不拢"| H["转人工"]
...
2. **提议与复核是两个不同厂商的模型。** 同一个模型自己复核自己等于没核。
3. **只有真分歧才转人工**——这就是 D4 第 4 步的出口。
4. **写入知识库不等于批准。** 通过流水线的 Claim 是 `candidate`；`approved` 需要人。目前 1,789 条里 6 条批准（见[第 9 节](#9-设计与实际不符之处)）。

与观点层是同一个形状：都由 `gpt-5.6-sol` 提议、Claude 复核、意见交回提议者仲裁。唯一的差别是复核模型的档位——这里是 `claude-sonnet-5`，观点层的新运行使用 `claude-fable-5-1`。
