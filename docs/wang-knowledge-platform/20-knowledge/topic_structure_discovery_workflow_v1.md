...
Topic Discovery 接入跨讲观点身份时，必须遵守 [Canonical Viewpoint Registry 与跨讲论证路径设计 v1](../20-knowledge/canonical_viewpoint_design.md)：`TopicNode` 继续拥有层级，`CanonicalViewpoint` 不成为另一棵主题树；runner 只消费 scoped、SHA-bound `ViewpointKnowledgeProjection`，并继续以原 Claim graph 验证无遗漏。
