...
Cross-sermon viewpoint identity, argument-route expansion, coverage disclosure and viewpoint dependency binding are governed by [Canonical Viewpoint Registry and cross-sermon argument-route design v1](../20-knowledge/canonical_viewpoint_design.md). Search and QA consume its scoped, SHA-bound `ViewpointKnowledgeProjection`; they do not infer canonical identity from similarity results or read the architecture Markdown at runtime.
