...
接下來的工作量把這個問題放大四十倍：**全庫都要走完講道線，之後在上面寫文章——已普查的兩百多篇是下限，尚有大量材料未整理。** 所以第一件產出物不是頁面，是**執行記錄表**。頁面讀表；表由 runner 寫。
...
勾選 → 選「跑到哪一階段為止」→ **先看試算，再決定**。每一篇從它缺的第一個階段接著跑，到指定的終點為止；已經是 ✓ 的階段預設跳過，要重跑得另外勾「連 ✓ 也重跑」。順序固定（抽取→複審→仲裁→合併→入庫），這不是工作流引擎，只是把固定的鏈條一次按完——數百篇一輪五個階段，逐階段手按是分批數乘五次，鏈起來是一次。文章不在鏈裡：它在文章總表，見第 7 節。
