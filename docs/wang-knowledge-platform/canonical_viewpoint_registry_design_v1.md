...
同日 patch-only v2 在第三个 clean temporary Registry 完成同一 `rock_referent_not_peter` E2E。原 proposal 与 Opus review 由相同 SHA-bound cache 复用，Sol 只输出 3 个 accepted component patches 与 2 个 candidate patches；deterministic merge 得到 13 Claims／27 components，CVP ChangeSet apply 81 operations，authority readback 后提交 Route job `RRJ-1c3310863190dfeff724`。Registry-backed Route run 对 14 个 current approved CVPs 一次生成 proposal，Opus 以 3 个 fixed review batches exact-once 覆盖 targets，Sol 一次 route correction resolved；最终 15 routes／20 source-local full attestations 通过，Route ChangeSet `KCS-a9181e9e1ac1de4b28a8` 在临时库 apply 50 creates，readback receipt `e605554678acccf5eb097961a34de7852f16db14e8fc3ed2b7f3b65f40c9285f` 为 0 record/CVP-cut mismatches。一个 terminal component 无 positive Registry link 的 attestation 及其孤立 route 被确定性隔离，没有阻断其余 routes；Opus 另提交 2 个 CVP possible-false-split re-review exceptions，Route 阶段未改写 CVP。首次 review resume 还验证 `evidence_claim_component_keys` 是 order-free ids：canonicalizer 排序／exact de-dup 后复用 raw review，不增加语义调用。production Registry 仍为原 1 个 CVP、0 新式 Claim links／ArgumentRoutes；所有 14 CVPs、15 routes、20 attestations 只存在 temporary test database。该结果完成 patch-only CVP → queue → Registry-backed Route → review/correction → apply/readback 的集成验证。
