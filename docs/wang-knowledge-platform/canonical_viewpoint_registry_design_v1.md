...
该结果只达到 `dual-model evidence-bound internal candidate`，不是 registry master record，也不是 human approval：`apply_allowed=false`、0 master-data mutations。只读 master promotion proposal 将 19 个 durable `ViewpointPropositionUnitRecord`、8 个 proposed membership links 与 11 个明确 excluded units exact-close，Claim-level membership links固定为 0；它还把 evidence packet、7 个 Claim revisions、source eligibility attestations、dual-model boundary 与 Article acceptance 绑定在同一 artifact 中。当前 proposal SHA-256 为 `17f0a181c236e4053f063c6db79abe42c49b265de1f5e27e1c7952ad7a958182`，targeted recall closure 已通过，剩余 blockers 是 formal atomic coverage snapshot、atomic resolution ledger、quality report 与 promotion approval。

旧 `ViewpointResolutionLedger` 以 Claim revision 为最小 row，不得直接用于 atomic promotion：本例至少一条 Claim 同时包含一个 viewpoint member unit 和一个 adjacent positive-interpretation unit；把整条 Claim 标为 `new_viewpoint_candidate` 或建立 `ViewpointClaimLink` 会吸收额外真值条件。正式 apply 前必须新增 PropositionUnit-level resolution rows，以 19-unit universe 为 denominator，对每个 unit exact-once 记录 member/excluded disposition、decision/boundary SHA 与 source evidence binding；quality report 必须验证该 atomic ledger、recall packet closure、consumer projection 与 master preview exact-equal。只有这些 formal artifacts 通过后，才能把 proposed records 转成 active/system-approved records。

下游 shadow contract 使用既有标准 `wang_viewpoint_knowledge_projection_v1`，而不是另造一套 Article-only schema。当前 recall-closed projection 位于 `matthew16-viewpoint-pilot-rock-v10-recall-closed-projection-2026-08-23/composition-projection.json`，SHA-256 为 `307a0770d3a0b5e9b149d2b12e19545a3627925af39391bc475b5b41a08898e0`；它含 1 个 candidate viewpoint、7 个 parent Claims、8 个 member PropositionUnits 与 26 个 evidence occurrences，并以 dependency manifest 绑定 candidate、Claim revisions 与 units。viewpoint row 明确投影 `knowledge_role=passage_interpretation`、`processing_phase=passage_exegesis` 与 `scripture_scope=[Matt.16.18]`。因为 projection eligibility 仍是 `internal_candidate`，非 dry-run 写作必须 fail closed；不能用 shadow validation 绕过 registry/quality/apply gate。
