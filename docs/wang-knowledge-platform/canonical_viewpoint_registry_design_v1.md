...
> 状态：Canonical Viewpoint layer 的规范性 architecture authority；基础 schema、review contract、projection、只读 UI、受控 cohort scheduler 与规则召回诊断已经实现。#194 已将首个太 16:18 释经观点通过 formal atomic gate 原子写入 PostgreSQL master data；文章、QA、搜索的 composition/public adapters 仍未接入。本文件不授权内容生成或部署。
> 版本：v1
> 日期：2026-08-22
...
下游 shadow contract 使用既有标准 `wang_viewpoint_knowledge_projection_v1`，而不是另造一套 Article-only schema。recall-closed projection 位于 `matthew16-viewpoint-pilot-rock-v10-recall-closed-projection-2026-08-23/composition-projection.json`，SHA-256 为 `307a0770d3a0b5e9b149d2b12e19545a3627925af39391bc475b5b41a08898e0`；它含 1 个 candidate viewpoint、7 个 parent Claims、8 个 member PropositionUnits 与 26 个 evidence occurrences，并以 dependency manifest 绑定 candidate、Claim revisions 与 units。viewpoint row 明确投影 `knowledge_role=passage_interpretation`、`processing_phase=passage_exegesis` 与 `scripture_scope=[Matt.16.18]`。该 artifact 仍是历史 shadow projection，eligibility 为 `internal_candidate`；master apply 不会静默把它升级为 composition。下一步必须从 active master records、atomic coverage/ledger/quality 与 automated decision 编译新的 `composition` projection，并接入 Matthew authoring runner；在此之前非 dry-run 写作继续 fail closed。
