...
首个 viewpoint 按 WIP=1 原则于 2026-08-23 完成垂直 candidate：`太 16:18 的「磐石」不指彼得本人`。这里的“完整”不是先挑几条看起来相同的 unit 再让模型确认；identity coverage invariant 要求 proposal reviewer（`gpt-5.6-sol/high`）与 blind reviewer（`claude-opus-5/high`）分别对本次 atomic decomposition 的**全部 17 个 PropositionUnits 逐项作 disposition**，返回的 unit set 必须与 execution universe exact-equal，任何遗漏、添加、重复、逐项分歧或 `unknown` 都阻断 synthesis。第一版人工预选 6 个 units 的 closed-set review 虽然两模型一致，却因未把 `教會不是建立在彼得本人身上` 送审而在 UI 暴露 false negative，因此作废为历史校准，不得用于 candidate。

修正后的 coverage run SHA-256 为 `81df9bc40b1f6a7047425947ea48436573d08511874f40fae0a69536abf83618`；两模型对 17/17 dispositions 完全一致，确定 7 个 same-truth-condition members、10 个 adjacent non-members、0 disagreements、0 unknowns。新 candidate 为 `CVP-547087bb6446897f56dd`，artifact SHA-256 为 `be1d9119db19d89a06fe026e433e97b99ab0ea3a1eebe3bd4513fe16c1e89961`。7 个 members 来自 6 个独立来源并全部绑定 current Claim revision、eligible EvidenceStep 与逐字 SourceFragment；Article 2 的精确句子 `教會不是建立在彼得這個人身上` 验收为 `supported`，但文章仍不是 source authority。10 个相邻非成员保留正面所指、语法理由、相关性与实质限定差异；例如 `教會不是單獨建立在彼得這個人身上` 只否定“单独”，并不等价于完全否定彼得本人为根基。
...
下游 shadow contract 使用既有标准 `wang_viewpoint_knowledge_projection_v1`，而不是另造一套 Article-only schema。加入明确的 `knowledge_classification` 后，当前 `composition_plan` projection 位于 `matthew16-viewpoint-pilot-rock-v3-classified-projection-2026-08-23/composition-projection.json`，SHA-256 为 `a4bfe735487507293252f560e3680c5aab491cbfe8653ceac1ef0ca0fcb7dcfb`；它只含 1 个 candidate viewpoint、6 个 parent Claims、7 个 member PropositionUnits、32 个 evidence occurrences 及其 fragment/source/citation rows，并以 dependency manifest 绑定 candidate、Claim revisions 与 units，不读取全 registry。viewpoint row 明确投影 `knowledge_role=passage_interpretation`、`processing_phase=passage_exegesis` 与 `scripture_scope=[Matt.16.18]`；分类前的 projection SHA `ee381d8f18943e661f1b267da7e3095298604381251a9aae2a80a82d07f47484` 已由当前分类 projection 取代。`matthew_exposition_authoring_runner --viewpoint-projection ... --dry-run` 在当前 PostgreSQL plan `CP-matthew-16-13-20` 与 current compiled knowledge 上实际返回 `inputs_valid`，packet SHA-256 `1c437b98fb39da2ce5384377a189fe2cd35ade8e80fad76bdf0b30148a77c75f`，0 model calls、no publication。因为 projection eligibility 仍是 `internal_candidate`，非 dry-run 写作会 fail closed；不能用 shadow validation 绕过 registry/quality/apply gate。历史 Article 2 snapshot 自身另有 sermon source SHA stale，故验证必须从 current store 编译 knowledge，不能复用那份过期 snapshot 假装 current。
