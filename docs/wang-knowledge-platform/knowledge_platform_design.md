...
Publication Profile 是用户批准、可以重复使用的出版体例，不是教授思想的一部分。它把“Carson style”一类要求转化为可以执行和审核的高层规则，而不是要求模型模仿某位作者的具体措辞。
...
3. **设计验证**：用释经讲座、专题检索档案、问答链、智能问答和方法研究检验同一知识结构。具体篇章编排只在对应的产品实验内出现。
...
8. **正式产品与公开 UI**：只有在共享模型和相应最小子图通过验证后，才把释经、专题、问答、搜索、智能问答、课程与研究作为公开产品发布。
