...
import { sourceEvidenceText } from "../viewpoints/VisualEvidence";
...
function DecisionBundle({ detail }: { detail: ExceptionDetail }) {
  return <article className="space-y-5 min-w-0">
    ...
    <section><h3 className="mb-3 font-black text-slate-950">来源证据</h3><div className="space-y-3">{detail.claims.map((claim) => <article key={claim.claim_id} className="rounded-xl border border-slate-200 bg-white p-4"><p className="font-bold leading-6 text-slate-900">{claim.statement}</p><p className="mt-1 font-mono text-[10px] text-slate-400">{claim.claim_id} · {claim.source_id}</p>{claim.evidence.map((evidence, index) => <blockquote key={index} className="mt-3 border-l-2 border-indigo-300 pl-3 text-sm leading-6 text-slate-700">{sourceEvidenceText(evidence)}</blockquote>)}</article>)}</div></section>
