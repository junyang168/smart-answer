...
if (!BACKEND_BASE) {
    throw new Error("SC_API_SERVICE_URL (or FULL_ARTICLE_SERVICE_URL) environment variable is required for admin email proxy");
}
...
function buildBackendUrl(pathSegments: string[] | undefined, search: string): string {
    ...
    return `${BACKEND_BASE}/admin/email${joined}${searchPart}`;
}
