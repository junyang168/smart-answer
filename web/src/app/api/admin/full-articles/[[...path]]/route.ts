...
if (!BACKEND_BASE) {
  throw new Error("FULL_ARTICLE_SERVICE_URL environment variable is required for full article proxy");
}
...
function buildBackendUrl(pathSegments: string[] | undefined, search: string): string {
  ...
  return `${BACKEND_BASE}/admin/full-articles${joined}${searchPart}`;
}
