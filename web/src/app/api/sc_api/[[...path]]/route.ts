...
if (!BACKEND_BASE) {
  throw new Error("SC_API_SERVICE_URL (or FULL_ARTICLE_SERVICE_URL) environment variable is required for sc_api proxy");
}
...
function buildBackendUrl(pathSegments: string[] | undefined, search: string): string {
  ...
  return `${BACKEND_BASE}/sc_api${joined}${searchPart}`;
}
