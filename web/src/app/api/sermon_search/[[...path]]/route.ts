...
if (!BACKEND_BASE) {
  throw new Error(
    "SC_API_SERVICE_URL (or FULL_ARTICLE_SERVICE_URL) environment variable is required for sermon_search proxy",
  );
}
...
function buildBackendUrl(pathSegments: string[] | undefined, search: string): string {
  const joined = pathSegments?.length
    ...
    : "";
  return `${BACKEND_BASE}/sermon_search${joined}${search || ""}`;
}
