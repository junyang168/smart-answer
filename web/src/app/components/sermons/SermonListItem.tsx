...
// 一個幫助函數，根據狀態返回不同的顏色
export const getSermonStatusClasses = (status: Sermon['status']) => {
    switch (status) {
    ...
    <Link href={`/resources/sermons/${sermon.id}`} className="block bg-white p-6 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300 border border-gray-200">
        <div className="flex justify-between items-start mb-2">
            ...
            <div className="flex gap-2">
                <span className={`text-xs font-semibold px-2 py-1 rounded-full ${getSermonStatusClasses(sermon.status)}`}>{sermon.status}</span>
            </div>
        </div>
        <div className="mb-3 flex flex-wrap gap-2">
            {sermon.organization_mode_label && (
            ...
            {sermon.series_title && (
                <span className="rounded-full bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-700">
                    {sermon.series_title}{sermon.series_order ? ` · 第 ${sermon.series_order} 講` : ''}
                </span>
            )}
        </div>
        <h3 className="text-xl font-bold font-display text-gray-800 mb-1">{sermon.title}</h3>
        {sermon.topic.length > 0 && (
            <p className="text-sm font-semibold text-blue-600 mb-2">{sermon.topic.slice(0, 4).join(' · ')}</p>
        )}
        {sermon.scripture.length > 0 && (
            <p className="font-semibold text-gray-600 mb-4">{sermon.scripture.slice(0, 3).join('；')}</p>
        )}
